...
• Playback: "Pause", "Resume", "Stop"`;
    ...
    if (cmd.includes('playlist') || cmd.includes('category') || cmd.includes('switch to')) {
        if (cmd.includes('generate') || cmd.includes('ai playlist')) {
            this.generateAIMoodPlaylist(true);
...
async generateAIMoodPlaylist(fromAIChat = false) {
    const mood = prompt('Tell Azaad AI your mood (e.g. chill, focus, workout, romantic):', 'chill');
    if (!mood) return;
    ...
    try {
        ...
        if (tracks.length === 0) {
            this.showToast('Azaad AI could not find songs for that mood.');
            return;
        }
        ...
        this.showToast('AI playlist generation failed. Try again.');
    }
}
